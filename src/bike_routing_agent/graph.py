...
def _after_route(state: RouteAgentState) -> str:
    return END if state.get("status") in _TERMINAL_AFTER_ROUTE else "score_candidates"
...
def build_graph(
    *,
    ...
    graph.add_conditional_edges("route_with_provider", _after_route, ["score_candidates", END])
